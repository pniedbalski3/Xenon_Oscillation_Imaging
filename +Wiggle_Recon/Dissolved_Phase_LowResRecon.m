...
kernelObj = Wiggle_Recon.Recon.SysModel.Kernel.Gaussian(kernel.sharpness, kernel.extent, verbose);
proxObj = Wiggle_Recon.Recon.SysModel.Proximity.L2Proximity(kernelObj, verbose);
clear kernelObj;
systemObj = Wiggle_Recon.Recon.SysModel.MatrixSystemModel(traj_redim, overgrid_factor, ...
    output_image_size, proxObj, verbose);

% Choose density compensation function (DCF)
dcfObj = Wiggle_Recon.Recon.DCF.Iterative(systemObj, nDcfIter, verbose);

% Choose Reconstruction Model
reconObj = Wiggle_Recon.Recon.ReconModel.LSQGridded(systemObj, dcfObj, verbose);
clear modelObj;
clear dcfObj;
